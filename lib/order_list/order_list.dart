...
class _OrderListState extends State<OrderList> {
            ...
            child: Column(
              children: [
                ...
                ...widget.selectedItems
                    .map(
                      (item) => buildOrderTile(
                        widget.selectedItems.indexOf(item),
                        item.title,
                        item.imagePath,
                        "${item.price}   |   Qty.1", // You can add quantity logic later
                      ),
                    )
                    .toList(),
  ...
  List<Widget> _buildPaymentSummary() {
    // Calculate subtotal by parsing prices
    double subtotal = widget.selectedItems.fold(0, (sum, item) {
      return sum + double.parse(item.price.replaceAll("\$", ""));
    });

    double discount = 9.99; // You can make this dynamic
